document.addEventListener('DOMContentLoaded', () => {
   ...
   // 2. Fonction qui lance la recherche
    const performSearch = () => {
        ...
        // Si la barre est vide ET qu'aucun tag n'est coché, on réaffiche tout
        ...
        // Si on tape au clavier, on remet le bouton alphabet sur "Tous"
        if (searchTerm !== '') {
       ...
       // 2A. Filtrer les données (Barre de texte + Menu Tags)
        let filteredData = lexiqueData.filter(item => {
            ...
            // Le mot ne s'affiche que s'il valide les deux filtres !
    ...
    function filterByLetter(letter) {
        ...
        // Met à jour visuellement les boutons (on enlève 'active' partout et on le met sur le bon)
        document.querySelectorAll('.alpha-btn').forEach(btn => {
            btn.classList.remove('active');
        ...
        if (letter === 'Tous') {
            // Si "Tous", on affiche tout le tableau trié
            lexiqueData.sort((a, b) => a.terme.trim().localeCompare(b.terme.trim()));
            displayResults(lexiqueData);
        } else {
            // Sinon, on filtre par la première lettre
            const filteredData = lexiqueData.filter(item => {
                const terme = item.terme ? item.terme.trim().toUpperCase() : '';
                return terme.startsWith(letter);
            });
            filteredData.sort((a, b) => a.terme.trim().localeCompare(b.terme.trim()));
            displayResults(filteredData);
        }
    }

    // On lance la création des boutons au chargement
    setupAlphabet();
